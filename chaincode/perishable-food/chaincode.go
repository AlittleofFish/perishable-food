...
func (t *PerishableFood) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	case "updateOrderTemperature":
		return updateOrderTemperature(stub, args)
	// 更新订单状态
...
func createOrder(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if commodityId == "" || id == "" || deliverAddress == "" || deliverTime == "" || quantity == "" || buyerId == "" || sellerId == "" || orderTime == "" {
		return shim.Error("invalid args")
	}
	...
	if val, err := time.Parse("2006-01-02 15:04:05", deliverTime); err != nil {
...
// 更新订单温度列表
func updateOrderTemperature(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	// 检查参数的个数
	if len(args) != 3 {
		return shim.Error("not enough args.")
	}
	...
	if orderId == "" || temperature == "" || recordTime == "" {
		return shim.Error("invalid args")
	}
	...
	// 数据格式转换
	var formattedTime time.Time
	if val, err := time.Parse("2006-01-02 15:04:05", recordTime); err != nil {
		return shim.Error(fmt.Sprintf("formate deliverTime error: %s", err))
	} else {
		formattedTime = val
	}
...
func updateOrderStatus(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	*/
	if status == "Done" {
		...
		// 获取物流商账号
		logisticsResult, logisticsErr := getStateByPartialCompositeKey(stub, "2")
		if logisticsErr != nil {
			return shim.Error(fmt.Sprintf("query account error: %s", logisticsErr))
		}
		defer logisticsResult.Close()

		for logisticsResult.HasNext() {
			val, err := logisticsResult.Next()
			if err != nil {
				return shim.Error(fmt.Sprintf("query accounts error: %s", err))
			}
		...
		if sumD.Hours()/24 > 0.0 {
			...
			if len(order.TemperatureVariation) != 0 {
				...
				lossFee = lowTempDif*0.1*float64(order.Quantity*int64(order.Commodity.Price)) + highTempDif*0.2*float64(order.Quantity*int64(order.Commodity.Price))
			}
		...
		sellerAcc.Balance += totalPrice * 0.8
		logisticsAcc.Balance += totalPrice - totalPrice*0.8
		...
		// 序列化对象
		logisticsBytes, logisticsErr := json.Marshal(logisticsAcc)
		if logisticsErr != nil {
			return shim.Error(fmt.Sprintf("marshal logistics error %s", logisticsErr))
		}

		var logisticsKey string
		if val, err := stub.CreateCompositeKey("account", []string{"2"}); err != nil {
			return shim.Error(fmt.Sprintf("create key error %s", err))
		} else {
			logisticsKey = val
		}

		if err := stub.PutState(logisticsKey, logisticsBytes); err != nil {
			return shim.Error(fmt.Sprintf("put logistics account error %s", err))
		}
	}
